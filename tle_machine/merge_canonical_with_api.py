...
def mapped_player(api_player: dict[str, Any], mapping: dict[str, dict[str, Any]]) -> tuple[bool, str, str]:
    ...
    target = clean(item.get("sackmann_player_key"))
    if status not in MAPPED_STATUSES or not target:
        return False, "", status or "not_mapped"
    return True, target, status


def convert_api_match(match: dict[str, Any], mapping: dict[str, dict[str, Any]]) -> tuple[dict[str, Any] | None, str, dict[str, Any]]:
    ...
    w_ok, w_target, w_status = mapped_player(winner, mapping)
    l_ok, l_target, l_status = mapped_player(loser, mapping)
...
def find_duplicate(api_match: dict[str, Any], strict_idx: dict[str, list[dict[str, Any]]], date_players_idx: dict[str, list[dict[str, Any]]]) -> tuple[str, dict[str, Any] | None]:
    ...
    if len(date_candidates) > 1:
        # Rare, but keep safe: only accept if exactly one candidate shares round or surface+level.
        ...
        refined = [m for m in date_candidates if clean(m.get("level")) == clean(api_match.get("level")) and clean(m.get("surface")) == clean(api_match.get("surface"))]
        if len(refined) == 1:
...
def add_to_indices(match: dict[str, Any], strict_idx: dict[str, list[dict[str, Any]]], date_players_idx: dict[str, list[dict[str, Any]]]) -> None:
...
def main() -> None:
    ...
    for match in iter_manifest_matches(args.sackmann_manifest):
        ...
        if not ok:
            ...
            skipped_rows.append({"source": "sackmann", "reason": reason, "match_id": clean(match.get("match_id")), "date": clean(match.get("date"))})
            continue
    ...
    if args.api_manifest.exists():
        for raw_api in iter_manifest_matches(args.api_manifest):
            ...
            api_match, mapping_status, detail = convert_api_match(raw_api, mapping)
    ...
    selected.sort(key=lambda m: (
        clean(m.get("date")), clean(m.get("gender")), clean(m.get("level")), clean(m.get("surface")),
        norm(m.get("tourney_name")), norm(m.get("round")), clean(m.get("canonical_match_id")),
    ))
    ...
    common_fields = [
        "source", "reason", "duplicate_strategy", "date", "gender", "level", "surface", "tourney_name", "round",
        "api_match_id", "api_event_key", "winner_api_key", "winner_api_name", "winner_mapping_status", "winner_sackmann_key",
        "loser_api_key", "loser_api_name", "loser_mapping_status", "loser_sackmann_key", "sackmann_match_id",
    ]
    write_csv(args.report_dir / "api_merge_skipped.csv", skipped_rows, common_fields)
