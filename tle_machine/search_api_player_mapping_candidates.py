...
DEFAULT_MAPPING_REVIEW = Path("data/reports/api_tennis/player_mapping_review.csv")
DEFAULT_IMPACT_CSV = Path("data/reports/api_tennis/player_mapping_unmapped_impact.csv")
DEFAULT_OUTPUT_CSV = Path("data/reports/api_tennis/player_mapping_sackmann_candidate_search.csv")
DEFAULT_OUTPUT_JSON = Path("data/reports/api_tennis/player_mapping_sackmann_candidate_search.json")

MAPPED_STATUSES = {"auto_mapped", "manual_mapped", "mapped"}
...
def manifest_paths(manifest_path: Path) -> list[Path]:
    ...
    files = manifest.get("year_files") or manifest.get("files") or []
    paths: list[Path] = []
    for item in files:
...
def build_sackmann_players(manifest_path: Path) -> tuple[dict[str, dict[str, Any]], dict[str, list[str]], dict[str, list[str]], dict[str, list[str]]]:
    players: dict[str, dict[str, Any]] = {}
    match_counts: Counter[str] = Counter()
    last_by_gender: dict[str, list[str]] = defaultdict(list)
    norm_by_gender: dict[str, list[str]] = defaultdict(list)
    ...
    for key, p in players.items():
        p["sackmann_matches"] = int(match_counts[key])
        by_gender[p["gender"]].append(key)
        if p["last"]:
            last_by_gender[f"{p['gender']}|{p['last']}"].append(key)
        if p["name_norm"]:
            norm_by_gender[f"{p['gender']}|{p['name_norm']}"].append(key)

    return players, by_gender, last_by_gender, norm_by_gender
...
def api_name_parts(name: str) -> dict[str, Any]:
    ...
    if len(nts) >= 2 and len(nts[-1]) == 1 and len(nts[-2]) > 1:
        # V. C. Loureiro J. -> surname Loureiro, not J
        last = nts[-2]
...
def score_candidate(api_name: str, candidate: dict[str, Any]) -> tuple[float, str]:
    ...
    # full normalized exact match, useful if API gives full name
    if api["norm"] and api["norm"] == cand_norm:
...
def load_impact(path: Path) -> dict[str, dict[str, str]]:
    out: dict[str, dict[str, str]] = {}
    for r in read_csv(path):
        key = pick(r, "api_player_key", "player_key", "api_key")
        if key:
            out[key] = r
    return out
...
def is_relevant_row(row: dict[str, str]) -> bool:
    status = pick(row, "mapping_status", "status")
    if status in MAPPED_STATUSES:
        return False
    api_name = pick(row, "api_name", "name", "player_name")
    if not api_name or "/" in api_name or " & " in api_name:
        return False
    gender = pick(row, "gender")
    return gender in {"men", "women"}


def candidate_pool(api_name: str, gender: str, players: dict[str, dict[str, Any]], by_gender: dict[str, list[str]], last_index: dict[str, list[str]]) -> list[str]:
    api = api_name_parts(api_name)
    pool: set[str] = set()
    if api["last"]:
        pool.update(last_index.get(f"{gender}|{api['last']}", []))
    # if last name search fails, use same gender full pool as fallback for weird API names
    if len(pool) < 3:
        pool.update(by_gender.get(gender, []))
    return list(pool)


def priority_from_counts(row: dict[str, str], impact: dict[str, str]) -> str:
    def intval(*keys: str) -> int:
        for k in keys:
            v = pick(row, k) or pick(impact, k)
            if v:
                try:
                    return int(float(v))
                except Exception:
                    pass
        return 0
    atp = intval("atp_wta_count", "level_atp_wta", "atp_wta_matches")
    gs = intval("grand_slam_count", "level_grand_slam", "grand_slam_matches")
    chal = intval("challenger_count", "level_challenger", "challenger_matches")
    itf = intval("itf_count", "level_itf", "itf_matches")
    total = intval("match_count", "today_match_count", "total_matches") or atp + gs + chal + itf
    if atp or gs:
        return "high"
    if chal >= 3 or total >= 8:
        return "medium"
    return "low"
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Search Sackmann candidates for global unresolved API player mapping rows.")
    ap.add_argument("--sackmann-manifest", default=str(DEFAULT_SACKMANN_MANIFEST))
    ap.add_argument("--mapping-review", default=str(DEFAULT_MAPPING_REVIEW))
    ap.add_argument("--impact-csv", default=str(DEFAULT_IMPACT_CSV))
    ...
    players, by_gender, last_index, _norm_index = build_sackmann_players(Path(args.sackmann_manifest))
    review_raw = read_csv(Path(args.mapping_review))
    impact_by_key = load_impact(Path(args.impact_csv))
    review = [r for r in review_raw if is_relevant_row(r)]

    out_rows: list[dict[str, Any]] = []
    counters: Counter[str] = Counter()

    for r in review:
        api_key = pick(r, "api_player_key", "player_key", "api_key")
        api_name = pick(r, "api_name", "name", "player_name")
        gender = pick(r, "gender")
        status = pick(r, "mapping_status", "status")
        impact = impact_by_key.get(api_key, {})
        priority = priority_from_counts(r, impact)
        counters["review_players"] += 1
        counters[f"priority_{priority}"] += 1
        counters[f"status_{status or 'unknown'}"] += 1

        scored = []
        for key in candidate_pool(api_name, gender, players, by_gender, last_index):
            p = players[key]
        ...
        base_fields = {
            ...
            "priority": priority,
            "impact_total_matches": pick(impact, "match_count", "total_matches"),
            "impact_atp_wta": pick(impact, "atp_wta_count", "level_atp_wta"),
            "impact_grand_slam": pick(impact, "grand_slam_count", "level_grand_slam"),
            "impact_challenger": pick(impact, "challenger_count", "level_challenger"),
            "impact_itf": pick(impact, "itf_count", "level_itf"),
            "impact_qualifying": pick(impact, "qualifying_count", "level_qualifying"),
        }
        ...
        if best_score >= 0.975 and margin >= 0.015:
    ...
    fieldnames = [
        "api_player_key", "api_name", "gender", "mapping_status", "priority",
        "impact_total_matches", "impact_atp_wta", "impact_grand_slam", "impact_challenger", "impact_itf", "impact_qualifying",
        "rank", "sackmann_player_key", "sackmann_name", "sackmann_matches", "score", "margin", "method", "suggested_action", "suggested_override_json",
    ]
    write_csv(Path(args.output_csv), out_rows, fieldnames)
    report = {
        "status": "ok",
        "generated_at": now_utc_iso(),
        "mapping_review": str(args.mapping_review),
        "impact_csv": str(args.impact_csv),
        "sackmann_manifest": str(args.sackmann_manifest),
        "input_review_rows": len(review_raw),
        "unresolved_players_checked": len(review),
